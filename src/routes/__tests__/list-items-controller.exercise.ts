...
test('createListItem returns 400 error if user already has a list item for the given book', async () => {
  const user = buildUser({id: 'FAKE_USER_ID'})
  ...
  mockedListItemsDB.query.mockResolvedValueOnce([existingListItem])
